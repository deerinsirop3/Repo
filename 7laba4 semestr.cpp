...
int listSizeInBytes(const std::list<int>& list) { //показывает размер в байтах
...
int main()
{
    ...
    std::vector<int> v(100000);  // создаем интовый вектор стотысячный
    std::list<int> l(100000);   // и создаем лист стотысячный
    {
    ...
    }
    {
        ...
        // таймер //засекаем время записи в вектор
        auto start = std::chrono::high_resolution_clock::now(); //измерение скорости работы 
        for (int i = 0; i < 10000; ++i) { 
            v[i] = i;
        }
        auto end = std::chrono::high_resolution_clock::now(); //время на запись в вектор
        ...
        start = std::chrono::high_resolution_clock::now(); //засекаем время на прочтение вектора
        for (int i = 0; i < 10000; ++i) {
            v[i];
        }
        end = std::chrono::high_resolution_clock::now(); //выводит итоговое время для чтения
